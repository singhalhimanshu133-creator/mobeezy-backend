...
class APIBridge:
    ...
    def get_accounts_summary(self):
        try:
            ...
            for u in users:
                cid = u.get('User ID')
                c_ledg = [l for l in ledger if str(l.get('Customer ID')) == str(cid)]
                ...
                accs.append({"Customer ID": cid, "Customer Name": u.get('User Name'), "Total Sales": tot_sales, "Total Payments": tot_pay, "Balance": bal})
    ...
    def get_customer_statement(self, customer_id=""):
        try:
            users = db_core.read_table('UserMaster')
            ledger = db_core.read_table('LedgerMaster')
            u = next((x for x in users if str(x.get('User ID')) == str(customer_id)), None)
            if not u: return {"success": False, "error": "Customer not found"}
            
            c_ledg = [l for l in ledger if str(l.get('Customer ID')) == str(customer_id)]
            c_ledg.sort(key=lambda x: (str(x.get('Date','')), str(x.get('Created Time', '')), str(x.get('Ledger ID', ''))))
